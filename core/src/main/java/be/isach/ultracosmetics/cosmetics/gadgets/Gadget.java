...
public abstract class Gadget extends Cosmetic<GadgetType> implements Updatable {
	...
	protected void onEquip() {
		...
		itemStack = ItemFactory.create(getType().getMaterial(), getType().getData(), ammo + getType().getName(),
	...
	public void openAmmoPurchaseMenu() {
		...
		for (int i = 27; i < 30; i++) {
			inventory.setItem(i,
			                  ItemFactory.create(Material.EMERALD_BLOCK, (byte) 0x0, MessageManager.getMessage("Purchase")));
